...
from _env.config import (
    ...
    FRONTEND_TARGET, PACKAGE_TARGET, BUILD_DIR,
    BACKEND_SRC, BACKEND_TARGET, BACKEND_ENTRYPOINT, BACKEND_APPNAME,
    PRODUCT_NAME, EB_PLATFORM, PACKAGE_EXECUTABLE,
    LOG_DIR, LOG_FILE, PORT_ANGULAR, PORT_BACKEND,
)
...
def _run_build_backend() -> None:
    ...
    result = subprocess.run(
        [
            ...
            "--distpath", str(BACKEND_TARGET),
            "--workpath", str(BACKEND_TARGET / ".pyi-work"),
            "--specpath", str(BACKEND_TARGET / ".pyi-specs"),
            "--noconfirm", str(BACKEND_ENTRYPOINT),
        ],
        cwd=ROOT,
    )
...
@cli.command()
@click.option("--platform", "eb_platform", default=None,
              help="Electron builder platform flag (--mac, --win, --linux)")
@click.option("--extra", "eb_extra", default="",
              help="Extra electron-builder flags")
